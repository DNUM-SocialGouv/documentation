...
sa réponse arrive dès que le serveur a terminé de la traiter.

![sync.png](img/sync.png)

1. L'utilisateur utilise l'interface de l'application pour exécuter une action.
   1. Il attend alors le traitement de la requête.
3. Une requête est envoyée au serveur,
4. Le serveur la traite,
5. Le serveur renvoie une réponse,
6. L'utilisateur peut de nouveau continuer à utiliser l'application.

En revanche, dans le cas d'un processus asynchrone, le serveur répond qu'il a tenu compte de la requête lors de sa
réception. Cela lui permet de la traiter à un moment opportun et surtout de redonner la main à l'utilisateur, qui
peut continuer d'utiliser le produit.
...
3. La réponse n'est pas renvoyée aussitôt, mais peut être récupérée dans un second temps par l'utilisateur.
   1. Comme indiqué [plus bas](#quels-moyens-de-récupérer-une-information), la réponse peut également être renvoyée à
...
être indéfini, il faut récupérer dans un second temps les données les plus à jour. Cela peut être contraignant si ce
...
suite à la requête envoyée et peut être présentée à l'utilisateur. Dans le cas d'un processus asynchrone, il
faut prévoir un [mécanisme de récupération](#quels-moyens-de-récupérer-une-information) supplémentaire, qui peut
impacter l'expérience utilisateur et la complexité technique de l'application.

La gestion des erreurs est également l'un des désavantages. Il faut se poser les questions suivantes lors de la
conception du parcours utilisateur : comment notifier d'un traitement qui a échoué, faut-il permettre la reprise du
processus, etc. Cela peut avoir des impacts sur l'expérience utilisateur.
