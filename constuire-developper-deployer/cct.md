...
- Accessibilité RGAA : viser systématiquement le niveau AA à 100% de conformité, en combinant tests automatisés réguliers (axe-core, WAVE) et audits manuels exhaustifs sur tous les écrans et parcours ; produire une déclaration d'accessibilité publique mise à jour annuellement avec plan d'action correctif daté si écarts résiduels, et publier un schéma pluriannuel de mise en accessibilité.
...
- RGS (Référentiel Général de Sécurité) : pour les téléservices et applications critiques, appliquer les règles du RGS v2.0 en fonction du niveau de sécurité requis (déterminer via analyse de risque formelle selon la méthode EBIOS Risk Manager ou équivalent) ; mettre en œuvre les mesures de sécurité correspondantes et documenter la conformité avec maintien d'un dossier de sécurité à jour.
