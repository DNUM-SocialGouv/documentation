...
1. **Réutiliser** une solution existante déjà déployée en production :
    ...
    * Solution interministérielle (ex : DémarchesSimplifiées, PISTE@DGFIP)
2. **Adapter** une solution open source existante :
    * Solution interministérielle (ex: DématSocial étend DémarchesSimplifiées, Portail BI repose sur SitesFaciles)
    * Solution grand public (ex : N8N pour un agrégateur de données, Metabase pour un tableau de bord, KeyCloak comme socle d'EFP Connect et ProConnect)
