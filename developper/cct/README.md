...
* **Documentation architecturale** : documenter l'architecture logicielle avec des [Architecture Decision Records (ADR)](https://github.com/joelparkerhenderson/architecture-decision-record) et des [Diagrammes C4](https://c4model.com) (Context, Container, Component, Code) maintenus à jour pour les décisions structurantes, facilitant l'onboarding et les évolutions majeures.
...
* **Transactions explicites** : utiliser les niveaux d'isolation appropriés, gérer les rollbacks automatiques sur erreur, et limiter la durée des transactions pour éviter les locks prolongés.
...
* **Logging structuré** : pour tracer les événements métiers critiques et faciliter le diagnostic :
...
* **Dégradation gracieuse** : fallback sur cache, fonctionnalité réduite, messages utilisateur explicites) pour éviter les pannes totales et maintenir une expérience utilisateur acceptable même en mode dégradé.
* **Health checks exhaustifs** : liveness pour redémarrage, readiness pour traffic) avec vérification des dépendances critiques (BDD, services externes) ; collecter et exposer des métriques essentielles (taux d'erreur, latence p50/p95/p99, throughput) avec des dashboards opérationnels. Définir des SLO (Service Level Objectives) mesurables (ex: 99.9% uptime, p95 < 500ms) et des SLI (Service Level Indicators) pour les suivre.
* **Alertes progressives** : warnings pour déviations mineures, critiques pour incidents majeurs avec escalade automatique si non traité ; éviter le sur-alerting en définissant des seuils pertinents et en regroupant les alertes corrélées.
* **Sauvegardes régulières** et teste de restauration afin d'assurer un redémarrage maîtrisé en cas d'incident.
...
* **Pagination au niveau BDD** : pagination SQL (LIMIT/OFFSET ou curseurs) plutôt qu'en mémoire dans le backend, et à fortiori jamais côté frontend. Charger uniquement les données nécessaires à la page demandée et transmettre les métadonnées de pagination (total, page courante) pour une navigation efficace.
...
* **Architecture scalable** : prévoir une architecture scalable (composants stateless, équilibrage de charge, scalabilité horizontale), lorsque la charge ou la criticité l'exige, avec monitoring pour déclencher les actions de scale.
...
* **Vérification stricte des types** quand le langage le permet.
* **Supprimer le code mort** régulièrement via des outils automatisés adaptés au langage utilisé.
...
* **HTML sémantique** : balises appropriées, structure hiérarchique, métadonnées complètes (Open Graph, Twitter Cards, meta description) et attributs ARIA pour une accessibilité fiable. Tester régulièrement avec des lecteurs d'écran et des outils automatisés.
...
* **Content Security Policy (CSP) stricte** avec whitelisting des sources autorisées et configurer des cookies sécurisés (HttpOnly, Secure, SameSite) ; servir systématiquement l'application en HTTPS sur tous les environnements publics avec HSTS activé.
...
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| **Runtime/Langage**    | Node.js (LTS, version 24 minimum)<br>TypeScript (version 5+ avec configuration stricte)                                       |
...
| Catégorie              | Technologies                                                                                                                 |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| **Langage**            | Java 25+ (LTS), [JDK Temurin](jdk.md)<br>avec records, pattern matching, virtual threads                                   |
| **Framework Backend**  | Spring Boot 4.x+ avec Spring Framework 7.x+<br>Architecture en couches via Spring Data et Spring Security                    |
...
| **Authentification**   | Spring Security avec ProConnect (IHM) et JWT dans les cookies (APIs)                                                         |
